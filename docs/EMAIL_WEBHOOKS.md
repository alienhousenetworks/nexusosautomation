...
OctaOS supports receiving inbound emails through an HTTP webhook. This is particularly useful for allowing the Support AI to read and respond to customer emails dynamically without having to poll an inbox via IMAP.

## Supported Providers

OctaOS supports receiving inbound email payloads via `multipart/form-data` and standard JSON formats. We recommend using one of the following providers:
...
5. Set the **Destination URL** to your OctaOS webhook endpoint:
   `https://<your-octaos-domain>/api/v1/support/email/webhook/<your-tenant-id>`
...
You can monitor incoming payloads in your OctaOS dashboard under the specific tenant's Support Tickets view. When a new email arrives, it will automatically be parsed, linked to the `customer_contact`, and the Support Agent will generate a reply if auto-reply is enabled.
